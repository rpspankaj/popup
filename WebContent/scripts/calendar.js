...
function getDaysPerMonth(month, year) {
	/*
	 * Check for leap year. These are some conditions to check year is leap year
	 * or not... 1.Years evenly divisible by four are normally leap years,
	 * except for... 2.Years also evenly divisible by 100 are not leap years,
	 * except for... 3.Years also evenly divisible by 400 are leap years.
	 */
	if ((year % 4) == 0) {
...
function createCalender(year, month, day) {
	...
	while (d <= getDaysPerMonth(month, year)) {
		...
		if (targetEl)
			td.onclick = new Function('pickDate(' + year + ', ' + month + ', '
					+ d + ')');
		else
		...
		if (thisTag.tagName == 'SELECT') {
			if (thisTag.id != 'selectMonth' && thisTag.id != 'selectYear') {
				...
				(thisLeft >= calLeft && thisLeft <= calRight)
						|| (thisRight <= calRight && thisRight >= calLeft) || (thisLeft <= calLeft && thisRight >= calRight))
						&& (
